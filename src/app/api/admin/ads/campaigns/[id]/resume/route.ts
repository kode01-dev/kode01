...
export async function POST(
  ...
  try {
    ...
    if (!adminSession) {
      await logAuditEvent({
        eventType: 'ads.campaign.resume.failed.forbidden',
    ...
    if (campaignError || !campaign) {
      await logAuditEvent({
        eventType: 'ads.campaign.resume.failed.not_found',
        userId: actorUserId,
        path: auditContext.path,
        ipAddress: auditContext.ipAddress,
        userAgent: auditContext.userAgent,
    ...
    if (error) {
      await logAuditEvent({
        eventType: 'ads.campaign.resume.failed.campaign_update',
        userId: actorUserId,
        path: auditContext.path,
        ipAddress: auditContext.ipAddress,
        userAgent: auditContext.userAgent,
    ...
    if (campaign.owner_user_id) {
      try {
        await dispatchNotification({
          ...
          templateKey: 'ads_campaign_resumed',
          locale: 'en',
          title: `Campaign resumed: ${campaign.name}`,
          message: `Your campaign "${campaign.name}" is active again.`,
          link: '/advertise',
          metadata: {
        ...
        console.error('Failed to send campaign resumed notification:', notificationError);
      }
    }

    await logAuditEvent({
      eventType: 'ads.campaign.resume.success',
      userId: actorUserId,
      path: auditContext.path,
      ipAddress: auditContext.ipAddress,
      userAgent: auditContext.userAgent,
    ...
    await logAuditEvent({
      eventType: 'ads.campaign.resume.failed.internal_error',
      userId: actorUserId,
      path: auditContext.path,
      ipAddress: auditContext.ipAddress,
      userAgent: auditContext.userAgent,
