...
async function runAbandonedCartEmailsCron(request: Request) {
  const run = startCronRun(request, 'abandoned-cart-emails');
  ...
  try {
    ...
    const result = await processAbandonedCartEmailJobs();
...
export async function GET(request: Request) {
  return runAbandonedCartEmailsCron(request);
}
