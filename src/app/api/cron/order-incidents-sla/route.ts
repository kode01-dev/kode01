...
async function runOrderIncidentsSlaCron(request: Request) {
  const run = startCronRun(request, 'order-incidents-sla');
  try {
    ...
    if (incidents.length === 0) {
      logCronSucceeded(run, { checkedIncidents: 0, escalatedIncidents: 0, slaDays });
      return withCronHeaders(
        NextResponse.json({ success: true, checkedIncidents: 0, escalatedIncidents: 0, slaDays }),
        run,
      );
    }
...
export async function GET(request: Request) {
  return runOrderIncidentsSlaCron(request);
}
