...
def invoke_recap_upstream(payload: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    ...
    bearer = (os.getenv("CRON_SECRET") or os.getenv("SUPABASE_SERVICE_ROLE_KEY") or "").strip()
    internal_token = (os.getenv("EDGE_INTERNAL_AUTH_TOKEN") or "").strip()
    if not bearer:
        raise RuntimeError("CRON_SECRET or SUPABASE_SERVICE_ROLE_KEY is required for recap invocation")
